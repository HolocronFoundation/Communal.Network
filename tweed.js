...
function getMainContractJSON() {
  ...
  return [{
    ...
    "name": "user__filings__fileIndices",
    "outputs": [{
      "type": "uint256",
      "name": "out"
    }],
    ...
    "name": "user__filings__numberOfFiles",
    "outputs": [{
      "type": "uint256",
      "name": "out"
    }],
  ...
  return toCall.call().catch(function(error) {
    ...
    if (retryCount != 0) {
      ...
      return delay(500).then(function() {
        return callAndRetry(toCall, callType, retryCount - 1);
      })
    }
  })
}
...
async function loadReplyFeed(parentDiv, parentIndex, numberToLoad = num_replies, startIndex = -1) {
  ...
  while (messagesLoaded < numberToLoad && nextLoadIndex > 0) {
    checked = await callAndRetry(mainContract.methods.tweed__replyIndex(parentIndex, nextLoadIndex), 1).then(function(messageIndex) {
      return Promise.all([checkMessage(messageIndex, 0), messageIndex]);
    });
  ...
  if (nextLoadIndex != 0) {
    ...
    loadMore.onclick = function() {
      loadMore.outerHTML = '';
      loadReplyFeed(parentDiv, parentIndex, numberToLoad, nextLoadIndex);
    }
...
async function loadGeneralFeed(numberToLoad = num_display, startIndex = lastMessageNumber) {
  ...
  if (nextLoadIndex != 0) {
    ...
    loadMore.onclick = function() {
      loadMore.outerHTML = '';
      loadGeneralFeed(numberToLoad, nextLoadIndex);
    }
    loadMoreDiv.appendChild(loadMore);
...
async function loadUserFeed(address, numberToLoad = num_display, nextLoadIndex = lastMessageNumber) {
  ...
  if (nextLoadIndex >= 0) {
    ...
    loadMore.onclick = function() {
      loadMore.outerHTML = '';
      loadUserFeed(address, numberToLoad, nextLoadIndex);
    }
    loadMoreDiv.appendChild(loadMore);
    document.getElementById("feed").appendChild(loadMoreDiv);
  }
...
function replyTo(replyIDNode, img_reply, index, parentDiv) {
  ...
  textEntry.onfocus = function() {
    clearDefaultText(textEntry, 'Enter your reply here!');
  }
  ...
  newButton.onclick = function() {
    reply(replyIDNode, img_reply, replyNode, index, parentDiv, textEntry);
  }
